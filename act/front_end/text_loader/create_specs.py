#===- act/front_end/text_loader/create_specs.py - Text Specs ------------====#
...
#
# Purpose:
#   Creates embedding-space InputSpec and OutputSpec pairs for SST/Yelp text
#   classifiers whose verification graph starts after token embedding lookup.
#
#===---------------------------------------------------------------------===#

"""Specification creator for text verify-from-embeddings tasks."""
...
class TextSpecCreator(BaseSpecCreator):
    """Create verification specifications from text dataset-model pairs."""
    ...
    def __init__(
        ...
        """Initialize the text specification creator.
    ...
    def create_specs_for_data_model_pairs(
        ...
        *,
        ...
        """Create embedding-space specs for text datasets.
        ...
        Args:
            ...
            p_norm: Lp norm metadata carried by ``EMBEDDING_LP``.
        ...
        datasets = [find_text_dataset_name(name) for name in (dataset_names or ["sst"])]
        ...
        for dataset in datasets:
            examples = load_text_dataset(dataset, split)
            ...
            for model_name in models:
                ...
                if model_name != "embedding_classifier":
                    logger.warning("Skipping unsupported text model '%s'", model_name)
                    continue
                model = TextEmbeddingClassifier().eval()
    ...
    def _validate_and_filter_specs(
        ...
        """Validate text spec pairs against the embedding-space model."""
        valid_pairs = []
        for input_spec, output_spec in spec_pairs:
            ...
            if is_valid:
                ...
                logger.debug("Text spec validation failed: %s", errors)
        return valid_pairs


def create_text_specs(
    dataset_names: list[str] | None = None,
    model_names: list[str] | None = None,
    num_samples: int = 1,
) -> list[tuple[str, str, nn.Module, list[torch.Tensor], list[tuple[InputSpec, OutputSpec]]]]:
    """Create text specs with default creator settings.

    Args:
        dataset_names: Text datasets to load.
        model_names: Text model names to build.
        num_samples: Number of examples per dataset-model pair.

    Returns:
        Text specification tuples in the unified front-end format.
    """
    creator = TextSpecCreator()
