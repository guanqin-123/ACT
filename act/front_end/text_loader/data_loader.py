#===- act/front_end/text_loader/data_loader.py - Text Data Loading ------====#
...
#   embedding tensors, and samples correctly classified verification inputs.
#
#===---------------------------------------------------------------------===#

"""Text dataset utilities for verify-from-embeddings front ends."""
...
TEXT_DATASETS: dict[str, str] = {
    ...
    "text": "Alias for SST binary sentiment",
...
class TextVocabulary:
    ...
    def __init__(self, examples: Sequence[TextExample], embedding_dim: int = 8) -> None:
...
def list_text_datasets() -> list[str]:
    """List available text dataset names.

    Returns:
        Sorted dataset names and aliases.
    """
    return sorted(TEXT_DATASETS)


def find_text_dataset_name(name: str) -> str:
    """Resolve a text dataset or alias name.
    ...
    if key in TEXT_DATASETS:
        return key
    raise ValueError(f"Text dataset '{name}' not found; available: {list_text_datasets()}")


def load_text_dataset(dataset_name: str = "sst", split: str = "test") -> list[TextExample]:
    """Load a text dataset from disk or deterministic fixture data.

    Args:
        dataset_name: Text dataset name, currently ``sst`` or ``yelp``.
    ...
    """
    dataset = find_text_dataset_name(dataset_name)
...
def sample_correctly_classified(
    ...
    *,
...
) -> list[tuple[TextExample, torch.Tensor, int]]:
    ...
    """
    selected: list[tuple[TextExample, torch.Tensor, int]] = []
    _ = model.eval()
...
def _load_sst(root: Path, split: str) -> list[TextExample]:
    ...
    if split == "train":
        ...
        for line in rows:
            ...
            examples.append(TextExample(label=int(label), tokens=tokenize(text)))
        return examples
...
def _load_yelp(root: Path, split: str) -> list[TextExample]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            examples.append(TextExample(label=label, tokens=tokenize(text)))
    return examples


def _parse_sst_tree_line(line: str) -> TextExample | None:
    ...
    return TextExample(label=binary_label, tokens=tokens, word_labels=word_labels)


def _synthetic_examples(dataset: str) -> list[TextExample]:
    """Return deterministic binary sentiment examples for fixture mode."""
    if dataset == "yelp":
        return [
            TextExample(label=1, tokens=["great", "service", "and", "delightful", "food"]),
            TextExample(label=0, tokens=["bad", "service", "and", "awful", "food"]),
            TextExample(label=1, tokens=["excellent", "fresh", "meal"]),
        ]
    return [
        TextExample(label=1, tokens=["a", "great", "movie"]),
        TextExample(label=0, tokens=["a", "bad", "movie"]),
        TextExample(label=1, tokens=["excellent", "acting"]),
    ]
