    ...
    def _create_specs_for_single_instance(
        ...
        try:
            input_spec, output_spec = parse_vnnlib_to_specs(
                vnnlib_path, labeled_tensor=labeled_tensor
            )
            logger.info(f"Parsed VNNLIB specs: {input_spec.kind}, {output_spec.kind}")
        except Exception as e:
            logger.error(f"Failed to parse VNNLIB specs: {e}")
            return None

        # Backfill labeled_tensor.label from inferred y_true when label was None.
        # This ensures the fuzzer's PropertyChecker uses the correct per-sample
        # true label (extracted from the VNNLib disjunctive property) rather than
        # falling back to label=None which disables violation checking.
        from act.front_end.specs import OutKind

        if (
            labeled_tensor.label is None
            and output_spec.kind == OutKind.TOP1_ROBUST
            and output_spec.y_true is not None
        ):
            labeled_tensor.label = output_spec.y_true.cpu()
            logger.info(
                f"Backfilled labeled_tensor.label={output_spec.y_true.tolist()} "
                f"from inferred VNNLib true_label"
            )
        ...
        if validate_shapes:
            validated_pairs = self._validate_and_filter_specs(
                spec_pairs, pytorch_model, labeled_tensor.tensor
            )
...
def create_vnnlib_specs(
    ...
    return creator.create_specs_for_data_model_pairs(
        categories=categories, max_instances=max_instances
    )
