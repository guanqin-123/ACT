...
def parse_vnnlib_to_tensors(
    vnnlib_path: Path, input_shape: Optional[Tuple[int, ...]] = None
...
def parse_vnnlib_to_specs(
    vnnlib_path: Path, labeled_tensor: Optional["LabeledInputTensor"] = None
    ...
    try:
        ...
        input_shape = (
            labeled_tensor.tensor.shape if labeled_tensor is not None else None
        )
        ...
        if output_constraints:
            ...
            if true_label is None:
                # Try to infer true_label from disjunctive output property.
                # Pattern: (assert (or (and (>= Y_0 Y_k)) (and (>= Y_1 Y_k)) ...))
                # means class k is the "protected" class → true_label = k.
                true_label_inferred = _extract_true_label_from_disjunction(
                    content, num_outputs
                )
                if true_label_inferred is not None:
                    true_label = true_label_inferred
                    logger.info(
                        f"Inferred true_label={true_label} from VNNLib disjunctive property"
                    )
            ...
            if true_label is not None:
                ...
                output_spec = OutputSpec(
                    ...
                    y_true=true_label.clone()
                    if isinstance(true_label, torch.Tensor)
                    else torch.tensor([int(true_label)], dtype=torch.int64),
                )
            else:
                # Try to extract scalar output bounds (e.g., AcasXu prop_1: Y_0 >= 3.99)
                range_bounds = _extract_output_range_bounds(content, num_outputs)
                if range_bounds is not None:
                    lb_vals, ub_vals = range_bounds
                    logger.info(
                        f"Extracted output RANGE bounds from VNNLib scalar constraints"
                    )
                else:
                    lb_vals = [float("-inf")] * num_outputs
                    ub_vals = [float("inf")] * num_outputs
                output_spec = OutputSpec(
                    kind=OutKind.RANGE,
                    lb=torch.tensor(lb_vals, dtype=get_default_dtype()),
                    ub=torch.tensor(ub_vals, dtype=get_default_dtype()),
                )
...
def _extract_output_constraints(
    content: str, num_outputs: int
    ...
    logger.debug(
        "Output constraint extraction not fully implemented (requires full SMT-LIB parser)"
    )

    return constraints


def _extract_output_range_bounds(
    content: str, num_outputs: int
) -> Optional[Tuple[List[float], List[float]]]:
    """
    Extract scalar output bounds from simple (assert (>= Y_i <const>)) patterns.

    Handles AcasXu-style properties like:
        (assert (>= Y_0 3.991125645861615))
        (assert (<= Y_0 -0.5))

    Returns:
        (lb_list, ub_list) if any scalar output bounds found, else None.
    """
    lb = [float("-inf")] * num_outputs
    ub = [float("inf")] * num_outputs
    found = False

    # Pattern: (assert (>= Y_<idx> <value>))  →  lb[idx] = value
    for m in re.finditer(
        r"\(assert\s+\(>=\s+Y_(\d+)\s+([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*\)\)",
        content,
    ):
    ...
    # Pattern: (assert (<= Y_<idx> <value>))  →  ub[idx] = value
    for m in re.finditer(
        r"\(assert\s+\(<=\s+Y_(\d+)\s+([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*\)\)",
        content,
    ):
    ...
    return (lb, ub) if found else None


def _extract_true_label_from_disjunction(
    content: str, num_outputs: int
) -> Optional[int]:
    """
    Extract true_label from disjunctive classification robustness properties.

    Detects the VNNLib pattern:
        (assert (or (and (>= Y_0 Y_k)) (and (>= Y_1 Y_k)) ... (and (>= Y_n Y_k))))
    where every disjunct compares some Y_j against the same Y_k.
    This encodes "the model must predict class k" — i.e., k is the true label.

    Also handles AcasXu-style safety properties where the protected output
    index is consistently the RHS of all >= comparisons.

    Args:
        content: Raw VNNLIB file content.
        num_outputs: Number of output variables.

    Returns:
        The protected class index k, or None if the pattern is not found.
    """
    # Match all (>= Y_<lhs> Y_<rhs>) occurrences in the output assertion block
    pattern = r"\(>=\s+Y_(\d+)\s+Y_(\d+)\)"
    matches = re.findall(pattern, content)

    if not matches:
        return None

    # Collect all RHS indices — the "protected" class appears as RHS in every clause
    rhs_indices = set(int(rhs) for _, rhs in matches)

    # If all comparisons share exactly one RHS index, that is the true label
    if len(rhs_indices) == 1:
        true_label = rhs_indices.pop()
        if 0 <= true_label < num_outputs:
            return true_label
