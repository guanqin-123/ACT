...
#   InKind: BOX, LINF_BALL, LIN_POLY, EMBEDDING_LP
...
class InputSpecLayer(nn.Module):
    """
    Batched input constraint checker (BOX, L∞, LIN_POLY, EMBEDDING_LP).
    ...
    def __init__(self, spec: Optional[InputSpec] = None, **kwargs):
        ...
        if self.kind == InKind.EMBEDDING_LP:
    ...
    def _validate_schema(self):
        ...
        if self.kind == InKind.EMBEDDING_LP:
            if self.p_norm is None:
                raise ValueError("EMBEDDING_LP requires p_norm")
            if self.perturbed_positions is None:
                raise ValueError("EMBEDDING_LP requires perturbed_positions")
    ...
    def forward(self, x: torch.Tensor):
        ...
        elif self.kind == InKind.EMBEDDING_LP:
            if self.lb is None or self.ub is None:
                return (x, True, "⚠️ INPUT EMBEDDING_LP: Missing lb/ub")

            lb_ok = (x >= self.lb).reshape(batch_size, -1).all(dim=1)
            ub_ok = (x <= self.ub).reshape(batch_size, -1).all(dim=1)
            satisfied = lb_ok & ub_ok
            n_ok = satisfied.sum().item()
            explanation = f"✅ INPUT EMBEDDING_LP: {n_ok}/{batch_size} satisfied"
            return (x, satisfied, explanation)
