...
class InputSpec:
    ...
    def materialize_box_seed(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.kind == InKind.EMBEDDING_LP:
            if self.center is None or self.eps is None:
                raise ValueError("EMBEDDING_LP requires both center and eps")
    ...
    def _embedding_position_mask(self, center: torch.Tensor) -> torch.Tensor:
        ...
        """
        if center.dim() < 2:
            raise ValueError("EMBEDDING_LP center must have at least [L, D] shape")
        ...
        if (index_positions < 0).any() or (index_positions >= token_count).any():
            raise ValueError(
                "EMBEDDING_LP perturbed_positions contains out-of-range token "
    ...
    def _normalize_embedding_bool_mask(
        ...
        raise ValueError(
            "EMBEDDING_LP boolean perturbed_positions must match "
