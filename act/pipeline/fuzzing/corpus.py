...
class FuzzingSeed:
    ...
    def __init__(
        ...
        self.original_tensor = (
            original_tensor if original_tensor is not None else tensor.clone()
        )
        self.original_index = (
            original_index
        ...
        self.label = (
            label if label is not None else torch.full((B,), -1, dtype=torch.long)
        )
...
class SeedCorpus:
    ...
    def __init__(
        self, initial_seeds: List[LabeledInputTensor], strategy: str = "energy"
    ):
        """
        ...
        for i, labeled_tensor in enumerate(initial_seeds):
            ...
            label_val = (
                int(labeled_tensor.label.item())
                if isinstance(labeled_tensor.label, torch.Tensor)
                else labeled_tensor.label
            )
    ...
    def add(self, seeds: FuzzingSeed, mask: torch.Tensor):
        ...
        self._original_tensors = torch.cat(
            [self._original_tensors, seeds.original_tensor[idx]], dim=0
        )
        self._original_indices = torch.cat(
            [self._original_indices, seeds.original_index[idx]]
        )
        ...
        self._select_counts = torch.cat(
            [
                self._select_counts,
                torch.zeros(len(keep), dtype=torch.long, device=self._device),
            ]
        )
