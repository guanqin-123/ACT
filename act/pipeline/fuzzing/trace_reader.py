...
def print_summary(reader: TraceReader):
    ...
    print(f"   Iterations: {summary['iterations_range'][0]} → {summary['iterations_range'][1]}")
...
def print_trace_detail(trace: Dict[str, Any], idx: int):
    ...
    print(f"📊 Coverage: {trace.get('coverage', 0.0):.2%} (Δ {trace.get('coverage_delta', 0.0):+.2%})")
    ...
    try:
        if args.summary:
            ...
            if 0 <= idx < len(reader):
                ...
                print(f"✅ Exported trace #{idx} to {output}")
            else:
                print(f"❌ Index {idx} out of range [0, {len(reader)-1}]")
                sys.exit(1)
...
class TraceAnalyzer:
    ...
    def show_trace_detail(self, trace_idx: int) -> None:
        ...
        if trace_idx < 0 or trace_idx >= len(self.traces):
            print(f"❌ Invalid trace index: {trace_idx} (valid: 0-{len(self.traces)-1})")
            return
    ...
    def plot_input_comparison(self, trace_idx: int, axes=None) -> None:
        ...
        if 'input_before' not in trace or 'input_after' not in trace:
            print(f"❌ Trace #{trace_idx} missing input data (requires --trace-level 1+)")
            return
    ...
    def export_trace(self, trace_idx: int, path: str) -> None:
        ...
        print(f"✅ Exported trace #{trace_idx} to {path}")
