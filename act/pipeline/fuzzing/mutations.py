...
class FGSMMutation(MutationStrategy):
    ...
    def mutate(self, input_tensor, model, activations=None, label=None):
        ...
        perturb_size = (
            self.perturb_size.to(input_tensor.device)
            if isinstance(self.perturb_size, torch.Tensor)
...
class PGDMutation(MutationStrategy):
    ...
    def mutate(self, input_tensor, model, activations=None, label=None):
        ...
        if self.step_size is None:
            ...
            step_size = float((x_high - x_low).abs().max().item()) / max(
                self.num_steps, 1
            )
...
class MutationEngine:
    ...
    def __init__(
        ...
        self.last_strategy: Optional[str] = (
            None  # Track last mutation strategy for tracing
        )
        self.last_gradients: Optional[Dict[str, torch.Tensor]] = (
            None  # For Level 3 tracing: gradient capture
        )
        self.last_loss: Optional[float] = (
            None  # For Level 3 tracing: loss value capture
        )
    ...
    def _compute_adaptive_perturb_size(self) -> Union[float, torch.Tensor]:
        ...
        if self.input_spec.kind == InKind.BOX:
            ...
            print(
                f"[MutationEngine] Unsupported InputSpec kind '{self.input_spec.kind}', falling back to fixed perturb_size=0.01"
            )
            return 0.01
        ...
        if self.perturb_mode == "adaptive_scalar":
            ...
            print(
                f"  - perturb_scale: {self.perturb_scale} (fraction of range per perturbation)"
            )
            ...
            print(
                f"  - interpretation: Each mutation perturbation covers {self.perturb_scale * 100:.1f}% of the range"
            )
        ...
        # Compute per-dimension perturb_size tensor
            ...
            print(
                f"  - perturb_size range: [{perturb_size_tensor.min().item():.6f}, {perturb_size_tensor.max().item():.6f}]"
            )
            print(f"  - perturb_size mean: {perturb_size_tensor.mean().item():.6f}")
            print(
                f"  - steps_to_traverse: ~{1 / self.perturb_scale:.1f} steps per dimension"
            )
            print(
                f"  - interpretation: Each mutation perturbation covers {self.perturb_scale * 100:.1f}% of each dimension's range"
            )
    ...
    def mutate(self, seeds: "FuzzingSeed") -> torch.Tensor:
        ...
        if self.perturb_mode != "fixed" and self.input_spec is not None:
            if self.input_spec.kind == InKind.LINF_BALL:
                ...
                if _lb.shape[0] > 1:
                    _ix = seeds.original_index.clamp(max=_lb.shape[0] - 1).to(
                        self.device
                    )
            ...
            if self.perturb_mode == "adaptive_scalar":
                perturb_size = perturb_size.mean(
                    dim=list(range(1, perturb_size.dim())), keepdim=True
                )
            strategy.perturb_size = perturb_size

        # Apply mutation
        mutated = strategy.mutate(
            batch_input, self.model, self.activation_map, label=labels
        )
    ...
    def _project(
        self, tensor: torch.Tensor, seeds: "Optional[FuzzingSeed]" = None
    ) -> torch.Tensor:
        """
