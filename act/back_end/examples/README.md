...
```bash
# Generate all networks from act/config/networkGen.yaml
...
- `../../config/networkGen.yaml` - Complete network specifications with layer definitions
- `nets/*.json` - Generated network files (saved in nets/ subfolder)

## Architecture

The factory is extremely simple:
1. Reads complete network specifications from `act/config/networkGen.yaml`
