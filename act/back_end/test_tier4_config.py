...
def test_alpha_per_spec_cli_flag_threads_to_bab_config() -> None:
    ...
    assert cli_off.alpha_per_spec is False

    parsed_on = _parse_args(["--instance", "3995", "--alpha-per-spec"])
    cli_on = _to_cli_args(parsed_on)
    assert cli_on.alpha_per_spec is True
